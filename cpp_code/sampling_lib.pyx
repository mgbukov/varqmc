...
cdef extern from "sample_4x4.h":
    ...
    void int_to_spinstate_conv[T](const int,T ,np.int8_t []) nogil

    T rep_int_to_spinstate[T](const int,T ,np.int8_t []) nogil
    T rep_int_to_spinstate_conv[T](const int,T ,np.int8_t []) nogil

    #np.uint16_t rep_int_to_spinstate_conv(const int,np.uint16_t ,np.int8_t []) nogil
...
cdef class Neural_Net:
    ...
    def __init__(self,shapes,N_MC_chains,NN_type='DNN',seed=0):
        ...
        # print(self.apply_layer(self.params[0], self.spinstate_s_py).shape)
        # exit()
    ...
    def _init_NN(self,W_shape,NN_type):
        ...
        if NN_type=='DNN':
            self.N_symm=L*L*2*2*2 # no Z symemtry
            ...
            self.N_symm=2*2*2 # no Z, Tx, Ty symemtry
        ...
        self.W_fc_real, = init_params(rng,input_shape)[1]
        self.W_fc_imag, = init_params(rng,input_shape)[1]
    ...
    def evaluate(self, params, batch):
        ...
        #Re_z_fc, Im_z_fc = polynomial(Re_Ws, Im_Ws)
    
        
        # symmetrize
        log_psi   = jnp.sum(Re_z_fc.reshape(self.reduce_shape,order='C'), axis=[1])
        phase_psi = jnp.sum(Im_z_fc.reshape(self.reduce_shape,order='C'), axis=[1])
        # 
    ...
    cpdef object _evaluate_mod(self, object batch):
        ...
        Re_Ws = self.apply_layer(self.W_fc_real, batch)
        Im_Ws = self.apply_layer(self.W_fc_imag, batch)
